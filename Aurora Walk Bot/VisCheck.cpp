...
std::unique_ptr<BVHNode> VisCheck::BuildBVH(const std::vector<TriangleCombined>& tris) {
    ...
    Vector3g diff = bounds.max - bounds.min;
...
bool VisCheck::IntersectBVH(const BVHNode* node, const Vector3g& rayOrigin, const Vector3g& rayDir, float maxDistance, float& hitDistance) {
...
bool VisCheck::IsPointVisible(const Vector3g& point1, const Vector3g& point2)
{
    Vector3g rayDir = { point2.x - point1.x, point2.y - point1.y, point2.z - point1.z };
...
bool VisCheck::RayIntersectsTriangle(const Vector3g& rayOrigin, const Vector3g& rayDir, const TriangleCombined& triangle, float& t)
{
    const float EPSILON = 1e-7f;

    Vector3g edge1 = triangle.v1 - triangle.v0;
    Vector3g edge2 = triangle.v2 - triangle.v0;
    Vector3g h = rayDir.cross(edge2);
    float a = edge1.dot(h);
    ...
    float f = 1.0f / a;
    Vector3g s = rayOrigin - triangle.v0;
    float u = f * s.dot(h);

    if (u < 0.0f || u > 1.0f)
        return false;

    Vector3g q = s.cross(edge1);
