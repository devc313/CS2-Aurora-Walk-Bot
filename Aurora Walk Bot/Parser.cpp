...
Parser::Parser(const std::string& path) : DataPath(path) {
    ...
    for (size_t i = 0; i < TrianglesList.size(); ++i) {
        ...
        const std::vector<Vector3g>& vertices = VerticesList[i];
...
std::vector<std::vector<Vector3g>> Parser::GetVertices() {
    std::ifstream file(DataPath, std::ios::binary);
    if (!file.is_open()) {
        return {};
    }
    ...
    return ParseSection<Vector3g>(fileData.data(), fileSize, "m_Vertices");
}
